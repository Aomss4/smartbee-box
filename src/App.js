...
import { fetchBorrowingRecords } from './api';
import ReportTable from './ReportTable';
import { motion, AnimatePresence } from 'framer-motion'; 
import { ArrowLeft, Calendar, Download, Box, Factory, Circle, Users, DollarSign, LayoutGrid, List } from 'lucide-react';
...
function App() {
  ...
  const loadData = useCallback(async () => {
    if (!selectedMachine || isloading.current) return; 
    isloading.current = true;
    try {
      const startStr = `${startDate} 00:00:00`;
      const endStr = `${endDate} 23:59:59`;
      const res = await fetchBorrowingRecords(selectedMachine.token, startStr, endStr, page);
      if (res && res.status === 1) {
        const rows = res.data.rows || [];
        const sortedRows = rows.sort((a, b) => new Date(b.pay_time) - new Date(a.pay_time));
        setData(sortedRows);
        setTotalPage(res.data.pageCount || 1);
      }
    } catch (err) { console.error("Fetch error:", err); } 
    finally { setTimeout(() => { isloading.current = false; }, 500); }
  }, [selectedMachine, page, startDate, endDate]);

  const calculateGlobalSummary = useCallback(async () => {
    if (!selectedMachine) return;
    setIsSumming(true);
    try {
      const startStr = `${startDate} 00:00:00`;
      const endStr = `${endDate} 23:59:59`;
      const firstRes = await fetchBorrowingRecords(selectedMachine.token, startStr, endStr, 1);
      if (firstRes && firstRes.status === 1) {
        let allRows = [...firstRes.data.rows];
        const totalP = firstRes.data.pageCount;
        if (totalP > 1) {
          const promises = [];
          for (let i = 2; i <= totalP; i++) {
            promises.push(fetchBorrowingRecords(selectedMachine.token, startStr, endStr, i));
          }
          const results = await Promise.all(promises);
          results.forEach(res => {
            if (res && res.status === 1) allRows = [...allRows, ...res.data.rows];
          });
        }
        setGlobalSummary({
          totalValue: allRows.reduce((sum, item) => sum + (Number(item.price) || 0), 0),
          totalItems: allRows.reduce((sum, item) => sum + (Number(item.pay_num) || 0), 0),
          uniqueStaff: new Set(allRows.map(item => item.user_name)).size,
          allRows: allRows 
        });
      }
    } catch (err) { console.error("Summary error:", err); }
    finally { setIsSumming(false); }
  }, [selectedMachine, startDate, endDate]);

  useEffect(() => { loadData(); }, [loadData]);
  useEffect(() => { calculateGlobalSummary(); }, [calculateGlobalSummary]);
  ...
  // --- 1. ฟังก์ชันส่งออก CSV หน้าประวัติการเบิก ---
  const exportHistoryCSV = () => {
    ...
    globalSummary.allRows.forEach(row => {
      ...
      csvContent += `${date},${productName},${userName},${row.pay_num},${row.price}\n`;
    });
    downloadFile(csvContent, `History_${selectedMachine.name}_${startDate}.csv`);
  };

  // --- 2. ฟังก์ชันส่งออก CSV หน้าสรุปสินค้า (ตามรูป da332f.png) ---
        ...
        {!selectedMachine ? (
          ...
          <motion.div key="report" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} style={{ padding: '30px', maxWidth: '1300px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
              <button onClick={() => { setSelectedMachine(null); setData([]); }} style={{ display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid #ddd', background: '#fff', padding: '10px 20px', borderRadius: '12px', color: '#444', cursor: 'pointer', fontWeight: 'bold' }}><ArrowLeft size={20} /> กลับหน้าเลือกตู้</button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '25px' }}>
              <div style={summaryCardStyle}>
                ...
                <div><p style={labelStyle}>ยอดเงินรวมช่วงที่เลือก</p><h3 style={valueStyle}>{isSumming ? "..." : globalSummary.totalValue.toLocaleString()} <small>฿</small></h3></div>
              </div>
              <div style={summaryCardStyle}>
                <div style={{ background: '#ebf8ff', padding: '12px', borderRadius: '15px' }}><Box color="#3182ce" /></div>
                <div><p style={labelStyle}>จำนวนที่เบิก</p><h3 style={valueStyle}>{isSumming ? "..." : globalSummary.totalItems.toLocaleString()} <small>กล่อง</small></h3></div>
              </div>
              <div style={summaryCardStyle}>
                <div style={{ background: '#fff5f5', padding: '12px', borderRadius: '15px' }}><Users color="#e53e3e" /></div>
                <div><p style={labelStyle}>พนักงาน</p><h3 style={valueStyle}>{isSumming ? "..." : globalSummary.uniqueStaff.toLocaleString()} <small>คน</small></h3></div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: '25px', padding: '25px', background: '#fff', borderRadius: '20px', boxShadow: '0 5px 20px rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: '#f8f9fc', padding: '10px 15px', borderRadius: '12px', border: '1px solid #eee' }}>
                ...
                <input type="date" value={startDate} onChange={(e) => {setStartDate(e.target.value); setPage(1);}} style={dateInputStyle} />
                <span>|</span>
                <input type="date" value={endDate} onChange={(e) => {setEndDate(e.target.value); setPage(1);}} style={dateInputStyle} />
              </div>
              
              {/* --- ปุ่มส่งออกที่เปลี่ยนฟังก์ชันตามหน้า (Dynamic Button) --- */}
              {activeTab === 'list' ? (
                ...
                {activeTab === 'list' ? (
                    <motion.div key="list" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        ...
                        <div style={{ marginTop: '30px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '15px' }}>
                            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} style={pageButtonStyle}>ก่อนหน้า</button>
                            <div style={{ padding: '10px 25px', background: '#007bff', color: '#fff', borderRadius: '12px', fontWeight: 'bold' }}>หน้า {page} / {totalPage}</div>
                            <button onClick={() => setPage(p => Math.min(totalPage, p + 1))} disabled={page === totalPage} style={pageButtonStyle}>ถัดไป</button>
                        </div>
                    </motion.div>
                ) : (
...
const valueStyle = { margin: 0, color: '#2d3748', fontSize: '1.5rem', fontWeight: '800' };
...
const pageButtonStyle = { padding: '10px 25px', borderRadius: '12px', border: '1px solid #ddd', cursor: 'pointer', background: '#fff', fontWeight: 'bold' };
